...
Run a subset with, for example, `set FLD_CASES=J,S`.
...
The teaching model also does not establish a gravity-derived geostatic stress
field. Use its curves to compare formulations and load paths, not as design
predictions.

When using the included Clausen VUMAT, configure `call_mc.f`, `constants.txt`,
`FLD_UMAT_NSDV=1`, and `FLD_SDV_INIT=0`. The last setting explicitly
initialises its diagnostic return-region variable without requiring `SDVINI`.
